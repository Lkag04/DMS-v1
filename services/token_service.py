...
def create_token(
    ...
    existing = db.query(DieselToken).filter(
        DieselToken.token_uuid == token_uuid
    ).first()
...
def get_token(db: Session, token_uuid: str):
    ...
    return db.query(DieselToken).filter(
        DieselToken.token_uuid == token_uuid
    ).first()
...
def mark_token_redeemed(db: Session, token: DieselToken):
